...
class DroneLift:
    def __init__(self, config):
        ...
        self.height = config['DroneLift'].get('HBE_height', self.bottom_height)
        self.config = config
    ...
    def get_record(self, DC:drone_controller, PT:point_cloud, points:np.ndarray):
        ...
        X_up, Y_up, Z_up = PT.get_point_clouds_from_depth(
            intrinsic_matrix, 
            pose_matrix,
            depth,
            3)
    ...
    def get_record_down(self, DC:drone_controller, PT:point_cloud, points:np.ndarray):
        ...
        X_down, Y_down, Z_down = PT.get_point_clouds_from_depth(
            intrinsic_matrix, pose_matrix, depth, 3)

        X_down, Y_down, Z_down = PT.get_global_point_cloud(
            location, X_down, Y_down, Z_down)
